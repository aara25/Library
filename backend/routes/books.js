...
router.get("/", async (req, res) => {
  try {
    const fieldsToSearch = ["title", "author","accessId"].map((search) => ({
      [search]: {
    ...
    fs.createReadStream(filePath)
      ...
      .on('end', () => {
        newData = results.map((item) => {
          ...
          return {
            author: item.Author,
            count: (item.Availability === "YES" || item.Availability === "Yes") ? 1 : 0,
            accessId: item.Access_no,
  ...
  async function saveDataToMongoDB(data) {
    ...
    try {
      ...
      for (const item of newData) {
        const existingItem = await Book.findOne({ accessId: item.accessId });
      ...
      if (newItems.length > 0) {
        ...
        await Book.bulkWrite(newItems.map((item) => ({
          insertOne: {
            document: item
          }
        })));
